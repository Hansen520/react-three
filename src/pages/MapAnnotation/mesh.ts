...
loader.load('https://geo.datav.aliyun.com/areas_v3/bound/100000_full.json', function (data: any) {
    ...
    geojson.features.forEach((feature: any) => {
        ...
        const [x, y] = mercator(feature.properties.center) as any; // 获取中心点

        const loader = new THREE.TextureLoader();
        const texture = loader.load('/pos标注.png');
        const material = new THREE.SpriteMaterial({ map: texture, transparent: true });

        const annotation = new THREE.Sprite(material);
        annotation.scale.setScalar(15); // 放大10倍

        // const material = new THREE.SpriteMaterial({ color: 'orange' });

        // const annotation = new THREE.Sprite(material);
        // annotation.scale.setScalar(10);

        // 用SpriteText画并标出文字, 获取名字
